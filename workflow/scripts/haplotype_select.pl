...
my $IND=$ARGV[2];
my $humandb=$ARGV[3];
my $haplotype_selected_output_dir=$ARGV[4];
my $output_dir=$ARGV[5];
my $ref_version=$ARGV[6];


my $alu_seq = uc Bio::DB::Fasta->new("${humandb}/mobileElements/ALU.fa")->seq("ALU");
my $L1_seq = uc Bio::DB::Fasta->new("${humandb}/mobileElements/LINE1.fa")->seq("LINE1");
my $sva_seq = uc Bio::DB::Fasta->new("${humandb}/mobileElements/SVA.fa")->seq("SVA");
###############
my @args;
my $search_length=10000;
my $db = Bio::DB::Fasta->new("${humandb}/${ref_version}/ref/${ref_version}.fa");
...
open(my $fh, '>', "${output_dir}/$filename") or die "Could not open file '$filename' $!";
my $seq = $db->seq("chr$chr");
open(my $index1, '>', "${output_dir}/${IND}.g${hap}.fa.${chr}.delta") or die  "Could not open file";
open(my $true_SVs, '>', "${output_dir}/${IND}.g${hap}.fa.${chr}.SVs") or die  "Could not open file";

print $index1 "new\tnew_delta\n";

$i=0;
open($vcf, '<', "${haplotype_selected_output_dir}/${IND}.g${hap}.vcf.${chr}");
while(my $line = <$vcf>){
	chomp $line;
