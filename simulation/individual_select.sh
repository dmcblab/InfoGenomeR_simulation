...
do
	...
	chr_end=`tail -n 1 /DAS_Storage5/leeyh/1000G_SNP_indel_SV_integrated/$j.vcf | cut -f2`;
	cat /DAS_Storage5/leeyh/1000G_SNP_indel_SV_integrated/$j.vcf | awk -F "\t" 'BEGIN{srand();}{
		if($'$IND' != "."){
                split($'$IND',g, "|");
