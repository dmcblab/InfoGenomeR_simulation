...
for(my $chr_index = 0; $chr_index <=22; $chr_index++){
	...
	my $db = Bio::DB::Fasta->new("/DASstorage6/leeyh/BG_job/2017.08.25.simulation/10.g1.fa.${chr_name}");
	...
	$chr_number[$chr_index][0]=$chr_name;
        my $db = Bio::DB::Fasta->new("/DASstorage6/leeyh/BG_job/2017.08.25.simulation/10.g2.fa.${chr_name}");
...
for(my $SVs = 0; $SVs < 400; $SVs++){
	...
	if($SVs < 300 ){ ####### germline$$$
		...
		@line[1]=0;
		@line[2]=inf;
		while( @line[1]<1 || @line[2] > $chr_cumul[@line[3]][$#{@chr_cumul[@line[3]]}]){
			@line = ();
			@line[0]=$SV_hash{int(rand(3))};
			@line[3]=int(rand($#chr_start+1));
                        my $c=`Rscript rexp_500000.R`;my @c = ();@c=split(/\s+/, $c);
			my $SV_size = @c[1];
			@line[1]=1+int(rand($chr_cumul[@line[3]][$#{@chr_cumul[@line[3]]}]+1-$SV_size));
		...
		print $fh "@line[0]\t@line[1]\t@line[2]\t@line[3]\n";
                ...
                @line[3]=0;
                @line[4]=inf;
                while( ((@line[0] eq "balanced_translocation" || @line[0] eq "unbalanced_translocation") && (@line[5] == @line[6] || @line[1]<1 || @line[2] > $chr_cumul[@line[5]][$#{@chr_cumul[@line[5]]}] || @line[3]<1 || @line[4] > $chr_cumul[@line[6]][$#{@chr_cumul[@line[6]]}])) 
			||
		       ((@line[0] ne "balanced_translocation" && @line[0] ne "unbalanced_translocation") && (@line[1]<1 || @line[2] > $chr_cumul[@line[3]][$#{@chr_cumul[@line[3]]}]))){
			...
			switch(@line[0]){
				...
				case "balanced_translocation" {
	                               ...
	                               @line[1]=1+int(rand($chr_cumul[@line[5]][$#{@chr_cumul[@line[5]]}]+1-$SV_size));
	                               @line[2]=@line[1]+$SV_size-1;
	                               my $SV_size = 1000+int(rand(10000));
	                               @line[3]=1+int(rand($chr_cumul[@line[6]][$#{@chr_cumul[@line[6]]}]+1-$SV_size));
				...
				case "WGD"{
					...
					@line[2]=1;
	                               ...
	                               @line[3]=int(rand($#chr_start+1));
					my $c=`Rscript rexp_5000000.R`;my @c = ();@c=split(/\s+/, $c);
	                               my $SV_size = @c[1];
	                               @line[1]=1+int(rand($chr_cumul[@line[3]][$#{@chr_cumul[@line[3]]}]+1-$SV_size));
	                               @line[2]=@line[1]+$SV_size-1;
				}
...
`perl results_to_fa.pl $#chr_start`;
