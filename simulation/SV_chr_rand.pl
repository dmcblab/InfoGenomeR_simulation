...
for(my $SVs = 0; $SVs < 400; $SVs++){
	...
	if($SVs < 300 ){ ####### germline$$$
		...
		print $fh "@line[0]\t@line[1]\t@line[2]\t@line[3]\n";
                ...
                @line[3]=0;
                @line[4]=inf;
                while( (@line[0] eq "balanced_translocation" && (@line[5] == @line[6] || @line[1]<1 || @line[2] > $chr_cumul[@line[5]][$#{@chr_cumul[@line[5]]}] || @line[3]<1 || @line[4] > $chr_cumul[@line[6]][$#{@chr_cumul[@line[6]]}])) 
			||
		       (@line[0] ne "balanced_translocation" && (@line[1]<1 || @line[2] > $chr_cumul[@line[3]][$#{@chr_cumul[@line[3]]}]))){
